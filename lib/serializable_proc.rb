  ...
  protected
    ...
    class Proc #:nodoc#
      ...
      def initialize(block, owner)
        ...
        @oklass, @file, @line = owner.class, File.expand_path(file), line.to_i
        initialize_code_and_sexp
      end

      def ==(other)
        other.object_id == object_id or
          other.is_a?(@oklass) && other.to_s == to_s
      end
      ...
      private
              ...
              @code, @sexp = [
                unescape_magic_vars(
                  RUBY_2_RUBY.process(eval(sexp += frag)).
                    sub(/(#{@oklass}\.new|Proc\.new|proc)/,'lambda').
        ...
        def extract_sexp_args
          ...
          regexp = Regexp.new([
            '^(.*(', (
              case marker
              when /(#{@oklass}|Proc)/ then rq["s(:iter, s(:call, s(:const, :#{$1}), :new, s(:arglist)),"]
        ...
        def raw_sexp_and_marker
          regexp = /^(.*?(#{@oklass}\.new|lambda|proc|Proc\.new)?\s*(do|\{)\s*(\|([^\|]*)\|\s*)?)/m
          raw = raw_code
